    ...
    if(err){
        ...
        if(errorLogger.isTrustError(err)){
            if(err.errorStack){
                ...
                return res.status(err.statusCode).json(new AppResponse(err.statusCode, errorDescription))
            }
            return res.status(err.statusCode).json(new AppResponse(err.statusCode, err.message, err.logError))
        }else{
